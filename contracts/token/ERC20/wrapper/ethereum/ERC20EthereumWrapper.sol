...
abstract contract ERC20EthereumWrapper is ERC20Burnable {
    ...
    bytes32 public constant ERC20_ETHEREUM_WRAPPER_INTERFACE_ID =
        keccak256("ERC20EthereumWrapper");

    constructor() {
        console.log("Instantiating ERC20EthereumWrapper.");

        console.log(
            "ERC20_ETHEREUM_WRAPPER_INTERFACE_ID interface ID: %s",
            address(uint256(ERC20_ETHEREUM_WRAPPER_INTERFACE_ID))
        );
    ...
    function _unwrap(address payable account_, uint256 amountToUnwrap_)
        public
        virtual
    {
    ...
    function unwrapFrom(address payable account_, uint256 amountToUnwrap_)
        public
        virtual
    {
        require(_balances[Context._msgSender()] >= amountToUnwrap_);
        uint256 decreasedAllowance_ =
            allowance(account_, Context._msgSender()).sub(
                amountToUnwrap_,
